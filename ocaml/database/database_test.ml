...
module Tests =
  struct
    ...
    let make_vbd vm _ uuid =
      [(*    "ref", r; *) ("uuid", uuid); ("VM", vm); ("type", "user")]

    let expect_missing_row tbl r f =
      try f ()
      with
      | Db_exn.DBCache_NotFound ("missing row", tbl', r')
      when tbl' = tbl && r = r'
      ->
        ()
    ...
    let expect_missing_tbl tbl f =
      try f ()
      with
      | Db_exn.DBCache_NotFound ("missing table", tbl', "") when tbl' = tbl ->
        ()

    let expect_integrity_violation tbl fld v f =
      try f ()
      with
      | Db_exn.Integrity_violation (tbl', fld', v')
      when tbl' = tbl && fld' = fld && v' = v
      ->
        ()
    ...
    let expect_uniqueness_violation tbl fld v f =
      try f ()
      with
      | Db_exn.Uniqueness_constraint_violation (tbl', fld', v')
      when tbl' = tbl && fld' = fld && v' = v
      ->
        ()
    ...
    let expect_missing_uuid tbl uuid f =
      try f ()
      with
      | Db_exn.Read_missing_uuid (tbl', "", uuid')
      when tbl' = tbl && uuid' = uuid
      ->
        ()
    ...
    let expect_missing_column name f =
      try f ()
      with
      | Db_exn.DBCache_NotFound ("missing column", _, name')
      when name' = name
      ->
        ()
    ...
    let expect_missing_field name f =
      try f ()
      with
      | Db_exn.DBCache_NotFound ("missing field", name', _) when name' = name ->
        ()
      ...
      | Some {Ref_index.name_label= name_label'; uuid; _ref} ->
          ...
          let real_ref =
            if Client.is_valid_ref t key then
              ...
              Client.db_get_by_uuid t tblname key
          in
          let real_name_label =
    ...
    let check_many_to_many () =
      ...
      let foo_1 =
        Table.find "foo:1" (TableSet.find "foo" (Database.tableset db))
      in
    ...
    let check_events t =
      ...
      Printf.printf
        "===TEST=== Checking that the VM creation event is reported: " ;
      if List.exists (fun (table, r) -> table = "VM" && r = vm) created then
        Printf.printf "Pass\n"
      else (
      ...
      if List.mem_assoc name_label vm_updated then
        Printf.printf "Pass\n"
      else (
        Printf.printf "Fail\n" ; failwith "Event problem"
      ) ;
      ...
      let vm_updated = List.filter (fun (r, _) -> r = vm) updated in
      let vm_updated = List.map snd vm_updated in
      if List.mem_assoc "VBDs" vm_updated then
        Printf.printf "Pass\n"
      else (
        Printf.printf "Fail\n" ; failwith "Event problem"
      ) ;
      ...
      Printf.printf
        "===TEST=== Checking one-to-many after one-update of non-reference \
         field: " ;
      let vm_updated = List.filter (fun (r, _) -> r = vm) updated in
      let vm_updated = List.map snd vm_updated in
      if not (List.mem_assoc "VBDs" vm_updated) then
        Printf.printf "Pass\n"
      else (
        Printf.printf "Fail\n" ; failwith "Event problem"
      ) ;
      ...
      Printf.printf
        "===TEST=== Checking the VBD doesn't appear in the deleted list: " ;
      let deleted = get_deleted db g7 in
      if not (List.mem vbd deleted) then
        Printf.printf "Pass\n"
      else (
        Printf.printf "Fail\n" ; failwith "Event problem"
      ) ;
    ...
    let main in_process =
      ...
      if Client.is_valid_ref t invalid_ref then
        failwith "is_valid_ref <invalid_ref> = true" ;
      ...
      if not (Client.is_valid_ref t valid_ref) then
        failwith "is_valid_ref <valid_ref> = false, after create_row" ;
      Printf.printf "get_table_from_ref <valid ref>\n" ;
      ( match Client.get_table_from_ref t valid_ref with
      | Some "VM" ->
          ()
      | Some _ ->
          failwith "get_table_from_ref <valid ref> : invalid table"
      | None ->
          failwith "get_table_from_ref <valid ref> : None"
      ) ;
      ...
      if Client.db_get_by_name_label t "VM" name <> [valid_ref] then
        failwith "db_get_by_name_label <valid name label>" ;
      Printf.printf "read_field <valid field> <valid objref>\n" ;
      if Client.read_field t "VM" name_label valid_ref <> name then
        failwith "read_field <valid field> <valid objref> : invalid name" ;
      Printf.printf "read_field <valid defaulted field> <valid objref>\n" ;
      if
        Client.read_field t "VM" "protection_policy" valid_ref
        <> "OpaqueRef:NULL"
      then
        failwith
          "read_field <valid defaulted field> <valid objref> : invalid \
           protection_policy" ;
      ...
      if not (List.mem_assoc name_label fv_list) then
        failwith "read_record <valid table> <valid ref> 1" ;
