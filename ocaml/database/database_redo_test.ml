...
let recover redo_log schema db =
  ...
  let orig_db = db in
  let callback =
    ("test replay", fun update db -> Database.notify update orig_db)
  in
  Redo_log_replay.read_from_redo_log ~callback redo_log "recovered.db"
    recovered Test_schemas.schema ;
