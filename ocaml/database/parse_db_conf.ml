...
let make path = {dummy_conf with path}
  ...
  String.concat ""
    [
      Printf.sprintf
        ...
        r.path (from_mode r.mode) r.compress r.is_on_remote_storage
...
let to_mode s =
  ...
  | "no_limit" ->
      No_limit
  | _ ->
      failwith (Printf.sprintf "unknown mode: %s" s)
  ...
  try
    ...
    let read_block () =
      ...
      in
      {
        path
      ; mode=
          maybe_put_in "mode" (* key name *) No_limit
            (* default if key not present *) to_mode
          (* fn to conv string->mode type *)
      ; compress= maybe_put_in "compress" false bool_of_string
      ; is_on_remote_storage=
          maybe_put_in "is_on_remote_storage" false bool_of_string
      ; write_limit_period=
          maybe_put_in "write_limit_period" default_write_limit_period
            int_of_string
      ; write_limit_write_cycles=
          maybe_put_in "write_limit_write_cycles" default_write_cycles
            int_of_string
      ; other_parameters= !key_values
      ; (* the things remaining in key_values at this point are the ones we haven't parsed out explicitly above.. *)
        last_generation_count= Generation.null_generation
      }
    in
