...
module Table = struct
  ...
  let remove g key t =
    ...
    let new_len, new_deleted =
      if t.deleted_len + 1 < upper_length_deleted_queue then
        (t.deleted_len + 1, new_element :: t.deleted)
      else
  ...
  let fold_over_deleted since f t acc =
    let rec loop xs acc =
      ...
      | (created, deleted, r) :: xs ->
          let new_acc =
            if deleted > since && created <= since then
              f r {Stat.created; modified= deleted; deleted} acc
            else
              acc
          in
...
type common_key = Ref of string | Uuid of string

let string_of_common_key = function Ref x -> x | Uuid x -> x
...
module Manifest = struct
  type t = {schema: (int * int) option; generation_count: Generation.t}

  let empty = {schema= None; generation_count= Generation.null_generation}
  ...
  let touch f x = {x with generation_count= f x.generation_count}

  let next = touch (Int64.add 1L)

  let schema x = match x.schema with None -> (0, 0) | Some (x, y) -> (x, y)

  let update_schema f x = {x with schema= f x.schema}
end
...
module Database = struct
  ...
  let update f x = {x with tables= f x.tables}

  let set_generation g = update_manifest (Manifest.touch (fun _ -> g))

  let update_tableset f x = {x with tables= f x.tables}

  let update_keymap f x = {x with keymap= f x.keymap}

  let register_callback name f x = {x with callbacks= (name, f) :: x.callbacks}
      ...
      TableSet.fold
        (fun tblname _ tbl acc ->
          Table.fold
            (fun rf _ row acc ->
              let acc =
                KeyMap.add_unique tblname Db_names.ref (Ref rf) (tblname, rf)
                  acc
              in
              if Row.mem Db_names.uuid row then
    ...
    let tables =
      Schema.ForeignMap.fold
        (fun one_tblname rels tables ->
          List.fold_left
            (fun tables (one_fldname, many_tblname, many_fldname) ->
              ...
              let vm_to_vbds =
                Table.fold
                  (fun vbd _ row acc ->
                    let vm =
                      Schema.Value.Unsafe_cast.string (Row.find one_fldname row)
                    in
                    let existing =
                      if Schema.ForeignMap.mem vm acc then
                        Schema.ForeignMap.find vm acc
                      else
                        []
                    in
...
let update_one_to_many g tblname objref f db =
  if not (is_valid tblname objref db) then
    db
  else
    List.fold_left
      (fun db (one_fld, many_tbl, many_fld) ->
...
let update_many_to_many g tblname objref f db =
  if not (is_valid tblname objref db) then
    ...
    List.fold_left
      (fun db (this_fld, other_tbl, other_fld) ->
        let this_fld_refs =
          Schema.Value.Unsafe_cast.set (get_field tblname objref this_fld db)
        in
...
let set_field tblname objref fldname newval db =
  ...
  let need_other_table_update =
    ...
    match
      (Schema.one_to_many tblname schema, Schema.many_to_many tblname schema)
    with
    | [], [] ->
        false
    | o2m, m2m ->
