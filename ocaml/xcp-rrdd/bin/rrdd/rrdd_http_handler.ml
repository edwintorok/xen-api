...
let unarchive_rrd_handler (req : Http.Request.t) (s : Unixfd.t) _ =
...
let get_vm_rrd_handler (req : Http.Request.t) (s : Unixfd.t) _ =
...
let get_host_rrd_handler (req : Http.Request.t) (s : Unixfd.t) _ =
  ...
  Rrd_unix.to_fd ~json:(List.mem_assoc "json" query) rrd Unixfd.(!s)

(* A handler for putting the SR's RRD data into the Http response. *)
let get_sr_rrd_handler (req : Http.Request.t) (s : Unixfd.t) _ =
...
let get_rrd_updates_handler (req : Http.Request.t) (s : Unixfd.t) _ =
  let query = req.Http.Request.query in
  ...
  Unix.write Unixfd.(!s) (Bytes.unsafe_of_string reply) 0 (String.length reply) |> ignore
