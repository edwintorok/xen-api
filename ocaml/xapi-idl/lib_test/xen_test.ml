...
let old_vm_t =
  "{\"id\": \"bc6b8e8a-f0a5-4746-6489-2745756f21b2\", \"ssidref\": 0, \
   ...
   \"system-manufacturer\": \"Xen\", \"system-product-name\": \"HVM domU\", \
   \"system-version\": \"\", \"system-serial-number\": \"\", \"hp-rombios\": \
   \"\", \"oem-1\":\"Xen\", \"oem-2\": \
   \"MS_VM_CERT/SHA1/bdbeb6e0a816d43fa6d3fe8aaef04c2bad9d3e3d\"}, \"ty\": \
   [\"HVM\", {\"hap\": true, \"shadow_multiplier\": 1.000000, \"timeoffset\": \
   \"0\", \"video_mib\": 4, \"video\": \"Cirrus\", \"acpi\": true, \"serial\": \
   \"pty\", \"keymap\": \"en-us\", \"pci_emulations\": [], \
   \"pci_passthrough\": false, \"boot_order\": \"cd\", \"qemu_disk_cmdline\": \
   false, \"qemu_stubdom\": false}], \"suppress_spurious_page_faults\": false, \
...
let test_upgrade_rules () =
  ...
  let vm_t =
    match Rpcmarshal.unmarshal Xenops_interface.Vm.t.Rpc.Types.ty rpc with
    | Ok vm ->
        vm
    | Error (`Msg m) ->
        failwith (Printf.sprintf "Failed to unmarshal: %s" m)
  in
  Alcotest.(check string) "VM name" vm_t.Xenops_interface.Vm.name "unnamed"
