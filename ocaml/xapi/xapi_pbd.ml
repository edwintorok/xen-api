...
let assert_no_srmaster_key dev_cfg =
  let k = "SRmaster" in
  if List.mem_assoc k dev_cfg then
    raise
...
let get_active_vdis_by_pbd ~__context ~self =
  ...
  let vdis_in_sr =
    List.filter (fun vdi -> sr = Db.VDI.get_SR ~__context ~self:vdi) vdis
  in
  vdis_in_sr
...
let abort_if_storage_attached_to_protected_vms ~__context ~self =
  ...
  then
    ...
    let protected_vms =
      List.filter (fun (_, record) -> Helpers.is_xha_protected_r record) vms
    in
    List.iter
...
let check_sharing_constraint ~__context ~sr =
  if not (Db.SR.get_shared ~__context ~self:sr) then
    let pbds = Db.SR.get_PBDs ~__context ~self:sr in
    ...
    Xapi_clustering.with_clustering_lock_if_needed ~__context ~sr_sm_type
      __LOC__ (fun () ->
        ...
        if Helpers.i_am_srmaster ~__context ~sr then (
          ...
          Xapi_dr.remove_vdis_from_cache ~__context
            ~vdis:metadata_vdis_of_foreign_pool ;
