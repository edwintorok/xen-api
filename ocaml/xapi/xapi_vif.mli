...
val plug : __context:Context.db Context.t -> self:API.ref_VIF -> unit
(** Hotplug the VIF, dynamically attaching it to the running VM *)

val unplug : __context:Context.db Context.t -> self:API.ref_VIF -> unit
(** Hot-unplug the VIF, dynamically unattaching it to the running VM *)

val unplug_force : __context:Context.db Context.t -> self:API.ref_VIF -> unit
...
val move :
  __context:Context.db Context.t -> self:[`VIF] Ref.t -> network:[`network] Ref.t -> unit
(** Move a VIF to another Network. *)
...
val set_locking_mode :
  __context:Context.db Context.t -> self:[`VIF] Ref.t -> value:API.vif_locking_mode -> unit
(** Set the locking mode of this VIF.
 *  Update the firewall rules associated with this VIF, if it is plugged. *)

val set_ipv4_allowed :
  __context:Context.db Context.t -> self:[`VIF] Ref.t -> value:string list -> unit
(** Set the list of IPv4 addresses allowed to use this VIF. *)

val add_ipv4_allowed :
  __context:Context.db Context.t -> self:[`VIF] Ref.t -> value:string -> unit
