...
val wait_for_management_ip : __context:Context.db Context.t -> string
(** Block until an IP address appears on the management interface *)

val wait_for_clustering_ip :
  __context:Context.db Context.t -> self:API.ref_Cluster_host -> string
(** Block until an IP address appears on the given cluster host PIF *)

val on_dom0_networking_change : __context:Context.db Context.t -> unit
...
val run : __context:Context.db Context.t -> ?mgmt_enabled:bool -> unit -> unit
...
val reconfigure_himn : __context:Context.db Context.t -> addr:string option -> unit
