...
let as_maybe_db t =
  match t.kind with #db as kind -> Some {t with kind} | #nodb -> None

let as_no_db t = {t with kind= `Removed}
...
let __get_task_name : (__context:_ t -> API.ref_task -> string) ref =
...
let __destroy_task : (__context:_ t -> API.ref_task -> unit) ref =
...
let from_forwarded_task ?(http_other_config = []) ?session_id
  ...
  let kind = forwarded in
  let task_name =
    if Ref.is_dummy task_id then
      Ref.name_of_dummy task_id
    else
      !__get_task_name ~__context:(get_initial kind) task_id
  in
...
let make ?(http_other_config = []) ?(quiet = false) ?subtask_of ?session_id
    ...
    ?task_description ?(origin = Internal) (kind : [< kind]) task_name =
  ...
  let task_id, task_uuid =
    if task_in_database then
      match kind with
      | #db as kind ->
          !__make_task ~__context:(get_initial kind) ~http_other_config
            ?description:task_description ?session_id ?subtask_of task_name
      | #nodb ->
          invalid_arg "task_in_database requires a database"
    else
...
let make_subcontext ~__context ?task_in_database task_name =
  ...
  let subcontext =
    make ~subtask_of ?session_id ?task_in_database __context.kind task_name
  in
  let tracing =
...
let of_http_req ?session_id ?(internal_async_subtask = false) ~generate_task_for
    ~supports_async ~label ~http_req ~fd (kind : kind) =
  (* kind on input not necessarily same as kind on output, if the task is forwarded *)
  ...
  if internal_async_subtask then
    new_task_context ()
  else (* TODO: make task part of kind, so we don't need 2 matches *)
    ...
    | Some task_id -> (
      ...
      | #nodb ->
          invalid_arg "Forwarding requires having access to the DB"
    )
    | None ->
