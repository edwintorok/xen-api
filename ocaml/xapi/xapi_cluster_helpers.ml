...
module Pem = struct
  ...
  let update_tls_config ~__context ~verify self =
    ...
    let tls_config = { (init ~__context ~cn) with verify_tls_certs = verify } in
    let result = Cluster_client.LocalClient.upd_config (Xapi_clustering.rpc ~__context) dbg tls_config in
