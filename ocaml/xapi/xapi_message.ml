...
let timestamp_to_string f = Printf.sprintf "%0.5f" f
...
let of_xml input =
  ...
  let rec f () =
    ...
    | `Data dat ->
        ...
        | "name" ->
            message := {!message with API.message_name= dat}
        | "priority" ->
        ...
        | "body" ->
            message := {!message with API.message_body= dat}
        | "generation" ->
            gen := Int64.of_string dat
        | "ref" ->
            _ref := dat
        | _ ->
            failwith "Bad XML!"
  ...
  with e -> Backtrace.is_important e ; raise e
...
let ref_symlink () = Printf.sprintf "%s/refs" message_dir

let gen_symlink () = Printf.sprintf "%s/gen" message_dir
...
let handle_message ~__context message =
  try
    if not (Pool_features.is_enabled ~__context Features.Email) then
      info
        "Email alerting is restricted by current license: not generating email"
    else if Sys.file_exists !Xapi_globs.xapi_message_script then
      let output, log =
        Forkhelpers.execute_command_get_output
          !Xapi_globs.xapi_message_script
          [message]
      in
      debug "Executed message hook: output='%s' log='%s'" output log
    else
      info "%s not found, skipping" !Xapi_globs.xapi_message_script
  with e ->
    error "Unexpected exception in message hook %s: %s"
      !Xapi_globs.xapi_message_script
...
let cache_insert _ref message gen =
  with_lock in_memory_cache_mutex (fun () ->
      ...
      if !in_memory_cache_length > in_memory_cache_length_max then (
        in_memory_cache :=
          Listext.List.take in_memory_cache_length_default !in_memory_cache ;
...
let write ~__context ~_ref ~message =
  ...
  let message_gen () =
    ...
    let gen, _, _ =
      Pervasiveext.finally (fun () -> of_xml xi) (fun () -> close_in ic)
    in
    gen
  in
  let gen = ref 0L in
    ...
    try
      with_lock event_mutex (fun () ->
          let fd, basefilename, filename =
            ...
            let rec doit n =
              if n > 10 then
                failwith "Couldn't create a file"
              else
...
let create ~__context ~name ~priority ~cls ~obj_uuid ~body =
  ...
  let priority =
    if
      ...
      && List.mem_assoc name !Api_messages.msgList
    then
...
let get_real dir filter since =
  List.map
    (fun (_, r, m) -> (r, m))
    (get_real_inner dir filter (since_name_filter since))
...
let get_since_for_events ~__context since =
  ...
  let result =
    ...
    | None ->
        List.map
          (fun (ts, x, y) -> (ts, Xapi_event.Message.Create (x, y)))
...
let handler (req : Http.Request.t) fd _ =
  ...
  let check_query param =
    if not (List.mem_assoc param query) then (
      error "Xapi_message.handler: HTTP request for message lacked %s parameter"
        param ;
      Http_svr.headers fd (Http.http_400_badrequest ()) ;
      failwith
        (Printf.sprintf "Xapi_message.handler: Missing %s parameter" param)
    )
  in
    ...
    (fun __context ->
      try
        ...
        if not (Pool_role.is_master ()) then
          ...
          List.iter
            (function _, r, m -> ignore (write ~__context ~_ref:r ~message:m))
            messages ;
...
let send_messages ~__context ~cls ~obj_uuid ~session_id ~remote_address
  ...
  let request =
    Xapi_http.http_request ~subtask_of ~query ~body Http.Put
