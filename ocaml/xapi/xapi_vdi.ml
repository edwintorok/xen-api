...
let update_allowed_operations ~__context ~self : unit =
  update_allowed_operations_internal ~__context ~self ~sr_records:[]
    ~pbd_records:[] ()

let cancel_tasks ~__context ~self ~all_tasks_in_db ~task_ids =
  let ops = Db.VDI.get_current_operations ~__context ~self in
  let set value = Db.VDI.set_current_operations ~__context ~self ~value in
...
let create ~__context ~name_label ~name_description ~sR ~virtual_size ~_type
  ...
  if virtual_size < sm_vdi.virtual_size then
    info "sr:%s vdi:%s requested virtual size %Ld < actual virtual size %Ld"
      (Ref.string_of sR)
      (Storage_interface.Vdi.string_of sm_vdi.vdi)
      virtual_size sm_vdi.virtual_size ;
...
let introduce_dbonly ~__context ~uuid ~name_label ~name_description ~sR ~_type
  ...
  Db.VDI.create ~__context ~ref ~uuid ~name_label ~name_description
    ~current_operations:[] ~allowed_operations:[] ~is_a_snapshot ~snapshot_of
    ~snapshot_time ~sR ~virtual_size ~physical_utilisation ~_type ~sharable
    ~read_only ~xenstore_data ~sm_config ~other_config ~storage_lock:false
    ~location ~managed ~missing:false ~parent:Ref.null ~tags:[]
    ~on_boot:`persist ~allow_caching:false ~metadata_of_pool
    ~metadata_latest:false ~is_tools_iso:false ~cbt_enabled ;
  ref
...
let internal_db_introduce ~__context ~uuid ~name_label ~name_description ~sR
  ...
  let ref =
    introduce_dbonly ~__context ~uuid ~name_label ~name_description ~sR ~_type
      ~sharable ~read_only ~other_config ~location ~xenstore_data ~sm_config
      ~managed ~virtual_size ~physical_utilisation ~metadata_of_pool
      ~is_a_snapshot ~snapshot_time ~snapshot_of ~cbt_enabled
  in
...
let introduce ~__context ~uuid ~name_label ~name_description ~sR ~_type
  ...
  List.iter
    (fun vdi ->
      if
        Db.VDI.get_location ~__context ~self:vdi = location
...
let snapshot_and_clone call_f ~__context ~vdi ~driver_params =
  ...
  let call_snapshot () =
    ...
    let new_vdi = call_f (Ref.string_of task) sr' vdi_info in
  ...
  Db.VDI.set_name_label ~__context ~self:newvdi
    ~value:vdi_rec.API.vDI_name_label ;
  Db.VDI.set_name_description ~__context ~self:newvdi
    ~value:vdi_rec.API.vDI_name_description ;
  ...
  Db.VDI.set_other_config ~__context ~self:newvdi
    ~value:vdi_rec.API.vDI_other_config ;
  ...
  let newvdi =
    Storage_access.transform_storage_exn (fun () ->
        ...
        with Storage_interface.Storage_error (Unimplemented _) ->
          debug
            "Backend reported not implemented despite it offering the feature" ;
          raise
            (Api_errors.Server_error
...
let wait_for_vbds_to_be_unplugged_and_destroyed ~__context ~self ~timeout =
  ...
  let rec loop ~token ~remaining_vbds =
    ...
    if remaining_vbds <> [] && Mtime.Span.compare elapsed timeout < 0 then (
      ...
      in
      let remaining_vbds =
        Option.value ~default:remaining_vbds most_recent_vbds
      in
  ...
  debug
    "wait_for_vbds_to_be_unplugged_and_destroyed: finished, VDI %s has %d \
     VBD(s)"
    vdi_uuid
    (List.length remaining_vbds) ;
...
let _data_destroy ~__context ~self ~timeout =
  ...
  if vdi_type <> `cbt_metadata then (
    ...
    try
      destroy_and_data_destroy_common ~__context ~self
        ~operation:(`data_destroy timeout)
    with e ->
...
let clone ~__context ~vdi ~driver_params =
  Storage_access.transform_storage_exn (fun () ->
      ...
      with Storage_interface.Storage_error (Unimplemented _) -> (
        ...
        let newvdi =
          create ~__context ~name_label:a.Db_actions.vDI_name_label
            ~name_description:a.Db_actions.vDI_name_description
            ~sR:a.Db_actions.vDI_SR ~virtual_size:a.Db_actions.vDI_virtual_size
            ~_type:a.Db_actions.vDI_type ~sharable:a.Db_actions.vDI_sharable
            ~read_only:a.Db_actions.vDI_read_only
            ~other_config:a.Db_actions.vDI_other_config
            ~xenstore_data:a.Db_actions.vDI_xenstore_data ~sm_config:[] ~tags:[]
        in
        try
          ...
          Db.VDI.remove_from_current_operations ~__context ~self:newvdi
            ~key:task_id ;
          update_allowed_operations ~__context ~self:newvdi ;
          newvdi
        with e ->
...
let copy ~__context ~vdi ~sr ~base_vdi ~into_vdi =
  ...
  try
    let dst =
      if Db.is_valid_ref __context into_vdi then
        into_vdi
      else
        ...
        Helpers.call_api_functions ~__context (fun rpc session_id ->
            ...
            ( if src.API.vDI_on_boot = `reset then
                try
                  Client.VDI.set_on_boot ~rpc ~session_id ~self:new_vdi
                    ~value:`reset
                with _ -> ()
            ) ;
    ...
    if dst_r.API.vDI_read_only then (
      ...
      raise
        (Api_errors.Server_error (Api_errors.vdi_readonly, [Ref.string_of dst]))
    ) ;
    if dst_r.API.vDI_virtual_size < src.API.vDI_virtual_size then (
      error
        "VDI.copy: cannot copy a VDI (%s) of size %Ld into a VDI (%s) of size \
         %Ld"
        (Ref.string_of vdi) src.API.vDI_virtual_size (Ref.string_of dst)
        dst_r.API.vDI_virtual_size ;
      raise
        (Api_errors.Server_error
...
let set_metadata_of_pool ~__context ~self ~value =
  if Db.VDI.get_type ~__context ~self = `cbt_metadata then (
    error
      "VDI.set_metadata_of_pool: called with a VDI of type cbt_metadata (at %s)"
      __LOC__ ;
    raise
      (Api_errors.Server_error
         ( Api_errors.vdi_incompatible_type
...
let set_on_boot ~__context ~self ~value =
  ...
  in
  let vdi' =
    Db.VDI.get_location ~__context ~self |> Storage_interface.Vdi.of_string
  in
  let module C = Storage_interface.StorageAPI (Idl.Exn.GenClient (struct
    let rpc = Storage_access.rpc
  end)) in
...
let open_database ~__context ~self =
  ...
  try
    ...
    Xapi_session.create_readonly_session ~__context ~uname:"disaster-recovery"
      ~db_ref
  with e ->
    let error = Printexc.to_string e in
    let reason =
      match e with
      | Db_exn.DBCache_NotFound (_, _, _) ->
          "Database does not match local schema."
      | _ ->
          error
    in
    debug "Caught %s while trying to open database." error ;
    raise
      (Api_errors.Server_error (Api_errors.could_not_import_database, [reason]))
...
let change_cbt_status ~__context ~self ~new_cbt_enabled ~caller_name =
  if Db.VDI.get_cbt_enabled ~__context ~self <> new_cbt_enabled then (
    let task = Context.get_task_id __context in
    let sr = Db.VDI.get_SR ~__context ~self in
    let sr' =
      Db.SR.get_uuid ~__context ~self:sr |> Storage_interface.Sr.of_string
...
let enable_cbt ~__context ~self =
  ...
  change_cbt_status ~__context ~self ~new_cbt_enabled:true
    ~caller_name:"VDI.enable_cbt"

let disable_cbt =
  change_cbt_status ~new_cbt_enabled:false ~caller_name:"VDI.disable_cbt"

let set_cbt_enabled ~__context ~self ~value =
  if Db.VDI.get_cbt_enabled ~__context ~self <> value then (
    Db.VDI.set_cbt_enabled ~__context ~self ~value ;
...
let list_changed_blocks ~__context ~vdi_from ~vdi_to =
  ...
  in
  let vdi_from =
    Db.VDI.get_location ~__context ~self:vdi_from
    |> Storage_interface.Vdi.of_string
  in
  let vdi_to =
    Db.VDI.get_location ~__context ~self:vdi_to
    |> Storage_interface.Vdi.of_string
  in
  let module C = Storage_interface.StorageAPI (Idl.Exn.GenClient (struct
    let rpc = Storage_access.rpc
...
let _get_nbd_info ~__context ~self ~get_server_certificate =
  if Db.VDI.get_type ~__context ~self = `cbt_metadata then (
    error "VDI.get_nbd_info: called with a VDI of type cbt_metadata (at %s)"
      __LOC__ ;
    raise
      (Api_errors.Server_error
         ( Api_errors.vdi_incompatible_type
