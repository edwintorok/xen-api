...
let minimally_compliant_miami_database =
  "<database><manifest><pair key=\"schema_major_vsn\" value=\"5\"/><pair \
   ...
   value=\"103\"/></manifest><table name=\"SR\" /><table name=\"pool\" \
   /><table name=\"VBD_metrics\"/><table name=\"console\" /><table \
   name=\"host\" /><table name=\"VIF_metrics\"/><table name=\"user\" /><table \
   name=\"PBD\" /><table name=\"pool_patch\" /><table name=\"host_metrics\" \
   /><table name=\"VLAN\" /><table name=\"Bond\" /><table name=\"VTPM\" \
   /><table name=\"event\"/><table name=\"VBD\" /><table \
   name=\"VM_guest_metrics\" /><table name=\"VDI\" /><table \
   name=\"VM_metrics\"/><table name=\"task\"/><table name=\"VM\" /><table \
   name=\"crashdump\"/><table name=\"network\" /><table name=\"PIF\" /><table \
   name=\"host_patch\"/><table name=\"host_crashdump\"/><table name=\"SM\" \
...
let prepare_database_for_restore ~old_context ~new_context =
  ...
  let device_to_ref =
    let physical =
      List.filter
        (fun self -> Db.PIF.get_physical ~__context:old_context ~self)
        all_pifs
    in
    List.map
      (fun self -> (Db.PIF.get_device ~__context:old_context ~self, self))
      physical
  in
  ...
  (* The PIFs of the master host in the backup need their MAC addresses adjusting
        ...
        is found in the backup, to ensure that management is set up correctly.
...
let http_fetch_db ~master_address ~pool_secret =
  let request =
    ...
    |> Helpers.with_cookie pool_secret
  in
