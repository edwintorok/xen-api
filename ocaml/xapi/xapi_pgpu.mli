...
val update_gpus : __context:Context.db Context.t -> unit
(** Synchronise the PGPU objects in the database with the actual devices
 *  in the local host. *)

val add_enabled_VGPU_types :
  __context:Context.db Context.t -> self:API.ref_PGPU -> value:API.ref_VGPU_type -> unit
...
val set_GPU_group :
  __context:Context.db Context.t -> self:API.ref_PGPU -> value:API.ref_GPU_group -> unit
...
val assert_can_run_VGPU :
  __context:Context.db Context.t -> self:API.ref_PGPU -> vgpu:API.ref_VGPU -> unit
...
val mxgpu_vf_setup : __context:Context.db Context.t -> unit
 ...
 * VFs will be created. *)
val nvidia_vf_setup :
  __context:Context.db Context.t -> pf:API.ref_PCI -> enable:bool -> unit
