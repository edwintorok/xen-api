...
let refresh_internal ~__context ~self =
  ...
  let network =
    if Db.is_valid_ref __context pif.API.pIF_bond_slave_of then
      let master =
        Db.Bond.get_master ~__context ~self:pif.API.pIF_bond_slave_of
      in
...
let property_names_and_values = [("gro", ["on"; "off"])]

let default_properties = [("gro", "on")]
...
let introduce_internal ?network ?(physical = true) ~t:_ ~__context ~host ~mAC
  ...
  in
  let metrics =
    match metrics with None -> make_pif_metrics ~__context | Some m -> m
  in
  let dbg = Context.string_of_task __context in
  ...
  *)
  if is_my_management_pif ~__context ~self:pif then (
...
let forget_internal ~t ~__context ~self =
  ...
  if List.mem_assoc device t.device_to_mac_table then
    warn "Forgetting PIF record even though device %s still exists" device ;
  ( try
...
let update_management_flags ~__context ~host =
  try
    let management_bridge =
      Xapi_inventory.lookup Xapi_inventory._management_interface
    in
    let management_networks =
      Db.Network.get_refs_where ~__context
        ~expr:(Eq (Field "bridge", Literal management_bridge))
    in
...
let introduce ~__context ~host ~mAC ~device ~managed =
  ...
  *)
  ...
  if List.mem device (List.map snd t.pif_to_device_table) then
    raise
      (Api_errors.Server_error (Api_errors.duplicate_pif_device_name, [device])) ;
  (* Assert that a network interface exists with *
     	 * the specified device name and MAC address. *)
  if not (List.mem (device, mAC) t.device_to_mac_table) then
    raise
      (Api_errors.Server_error
         ( Api_errors
  ...
  introduce_internal ~t ~__context ~host ~mAC ~device ~mTU ~vLAN:(-1L)
    ~vLAN_master_of:Ref.null ~managed ()
...
let set_property ~__context ~self ~name ~value =
  ...
  if
    (not (pif_has_properties ~__context ~self))
    || Db.PIF.get_bond_slave_of ~__context ~self <> Ref.null
  then
    raise
      (Api_errors.Server_error
  ...
  let vlan_pifs =
    List.map (fun self -> Db.VLAN.get_untagged_PIF ~__context ~self) vlans
  in
  List.iter
    (fun pif ->
      if Db.PIF.get_currently_attached ~__context ~self then
...
let rec unplug ~__context ~self =
  ...
  | Bond_master bond :: _ ->
      List.iter
        (fun slave ->
          if Db.PIF.get_sriov_physical_PIF_of ~__context ~self:slave <> [] then (
            debug
              "PIF is bond master, one of its slaves is a network SRIOV \
  ...
  | Network_sriov_logical _ :: _ ->
      debug
        "PIF is network SRIOV logical PIF, also bringing down vlan on top of it" ;
...
let rec plug ~__context ~self =
  ...
  let () =
    ...
    | Tunnel_access tunnel ->
        ...
        in
        if
          Db.PIF.get_ip_configuration_mode ~__context ~self:transport_PIF
          = `None
        then
          raise
            Api_errors.(
    ...
    | VLAN_untagged vlan ->
        ...
        if Db.PIF.get_sriov_logical_PIF_of ~__context ~self:tagged_pif <> []
        then (
          debug
            "PIF is VLAN master on top of SRIOV logical PIF, also bringing up \
             SRIOV logical PIF" ;
    ...
    | Physical pif_rec ->
        ...
        if bond <> Ref.null then
          if pif_rec.API.pIF_sriov_physical_PIF_of <> [] then (
            ...
            debug
              "PIF is SRIOV physical PIF and bond slave, also bringing up bond \
               master PIF" ;
