...
type disk = {
    device: string  (** device inside the guest eg xvda *)
...
let get_template_record rpc session_id vm =
  ...
  in
  let script =
    if List.mem_assoc post_install_key other_config then
      Some (List.assoc post_install_key other_config)
    else
      None
  in
  if disks = [] && script = None then
    None
  else
...
let create_disk rpc session_id vm sm_config disk =
  ...
  let vdi =
    Client.VDI.create ~rpc ~session_id ~name_label:""
      ~name_description:"Created by template provisioner" ~sR:sr
      ~virtual_size:disk.size ~_type:disk._type ~sharable:false ~read_only:false
      ~other_config:[] ~xenstore_data:[] ~sm_config ~tags:[]
  in
  let vbd_ref =
    Client.VBD.create ~rpc ~session_id ~vM:vm ~vDI:vdi ~userdevice:disk.device
      ~bootable:disk.bootable ~mode:`RW ~_type:`Disk
      ~unpluggable:(disk._type <> `system) ~empty:false ~qos_algorithm_type:""
