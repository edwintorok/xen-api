...
val destroy : __context:Context.db Context.t -> self:[`GPU_group] Ref.t -> unit
(** Destroy a GPU group. *)

val find_or_create : __context:Context.db Context.t -> [`PGPU] Ref.t -> [`GPU_group] Ref.t
(** Find a GPU group for a given pGPU, or create a new one. *)

val update_enabled_VGPU_types :
  __context:Context.db Context.t -> self:[`GPU_group] Ref.t -> unit
