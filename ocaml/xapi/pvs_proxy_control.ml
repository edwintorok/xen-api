...
module State = struct
  ...
  *)
  ...
  let remove_site ~__context site =
    let site_uuid = Db.PVS_site.get_uuid ~__context ~self:site in
    with_xs (fun xs -> xs.Xs.rm (root // site_uuid))
...
let start_proxy ~__context vif proxy =
  ...
  try
    ...
    Network.Net.Bridge.add_port dbg None bridge port_name [] None
      (Some Network_interface.PVS_proxy) ;
  ...
  with e ->
    let reason =
      ...
      | No_cache_sr_available ->
          ...
          let body =
            Printf.sprintf
              "Unable to setup PVS-proxy %s for VIF %s: no cache storage found \
               on PVS-site %s for host %s."
              proxy_uuid
      ...
      | Network_interface.(Network_error PVS_proxy_connection_error) ->
          ...
          let body =
            Printf.sprintf
              "Failed to setup PVS-proxy %s for VIF %s on host %s due to an \
               internal error"
              proxy_uuid
              (Db.VIF.get_uuid ~__context ~self:vif)
              (Db.Host.get_name_label ~__context
      ...
      | Helpers.No_pvs_server_available ->
          ...
          let body =
            Printf.sprintf
              "Unable to setup PVS-proxy %s for VIF %s: no PVS-server found on \
               PVS-site %s for host %s."
              proxy_uuid vif_uuid site_name_label host_name_label
          in
      ...
      | Api_errors.Server_error ("SR_BACKEND_FAILURE_79", _, _) ->
          ...
          let body =
            Printf.sprintf
              "Unable to setup PVS-proxy %s for VIF %s and PVS-site %s: PVS \
               cache size for host %s exceeds SR available space."
              proxy_uuid
              (Db.VIF.get_uuid ~__context ~self:vif)
              (Db.PVS_site.get_name_label ~__context
                 ~self:(Db.PVS_proxy.get_site ~__context ~self:proxy)
    ...
    warn
      "Unable to enable PVS proxy for VIF %s: %s. Continuing with proxy \
       unattached."
      (Ref.string_of vif) reason ;
    false
...
let find_proxy_for_vif ~__context ~vif =
  ...
  in
  match proxies with [] -> None | proxy :: _ -> Some proxy
...
let maybe_stop_proxy_for_vif ~__context ~vif =
  Option.iter (stop_proxy ~__context vif) (find_proxy_for_vif ~__context ~vif)
