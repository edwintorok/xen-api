...
module Make (Svc : S) = struct
  ...
  let get_state id ~check_health : Svc.state msg_or_trap_result * Svc.Config.t msg_or_trap_result =
    ...
    let state =
      trap_exn @@ fun () -> Svc.get_state_exn id
    in
  ...
  let stop_exn id current_state =
    ...
    | Error err, _ ->
        fdebug (fun m -> m "instance in unknown state, desired stop: %a" pp_msg_or_trap err);
    ...
    let ensure_started id current_state desired_config =
      ...
      | Error err, Ok current ->
        fdebug (fun m -> m "Cannot determine current state, killing: %a" pp_msg_or_trap err);
        trap_exn @@ fun () ->
      ...
      | _, Error err ->
        ...
        fdebug (fun m -> m "Cannot determine current configuration: %a. Killing based on new ID" pp_msg_or_trap err);
        Svc.stop_exn ~force:true id;
        fdebug (fun m -> m "Starting service");
        Svc.start_exn id desired_config
    ...
    | Ok (Stopping|Starting), Ok old ->
        trap_exn @@ fun () ->
        fdebug (fun m -> m "instance is stopping or starting with different config, but desired start. Killing.");
        Svc.stop_exn ~force:true id;
        Svc.start_exn id desired_config
  ...
  let set_state uuid desired_state =
    ...
    | None ->
      ...
      fdebug (fun m -> m "removing instance state, stopped: %a" Fpath.pp state_path);
    ...
    | Some desired ->
      ...
      fdebug (fun m -> m "instance started with configuration %a" Svc.Config.pp_dump desired);
