...
module MakeSTM(KV: KVDirect) : STM.Spec = struct
  ...
  let show_cmd = function
    | Get k -> Printf.sprintf "GET(%S)" @@ key_to_string k
    | Put (k, v) -> Printf.sprintf "PUT(%S, %S)" (key_to_string k) (value_to_string v)
    | Delete k -> Printf.sprintf "DELETE(%S)" (key_to_string k)
    | List -> "LIST"
  
  let run cmd sut =
    match cmd with
    | Get g -> Res (option value, KV.get sut g)
    | Put (k, v) -> Res(option value, KV.put sut k v)
    | Delete k -> Res(option value, KV.delete sut k)
    | List -> Res (list key, KV.list sut)
  ...
  | Get _ | List -> state (* queries do not change state *)
  | Put (k, v) -> KeyMap.add k v state
  | Delete k -> KeyMap.remove k state
  
  let precond _cmd _state = true
  
  let postcond cmd (state:state) res =
    match cmd, res with
    | (Get k | Put (k, _) | Delete k), Res((Option Value, _), v) ->
  ...
  let shrink_cmd =
    let open QCheck in
    function
    | Get (Types.Key k) -> Iter.map (fun k -> Get (Types.Key k)) (Shrink.string k)
    | Put (Types.Key k, Types.Value v) ->
      Iter.map2 (fun k v -> Put (Types.Key k, Types.Value v))
  ...
  let small_cmd = function
    | Get (Types.Key k) | Delete (Types.Key k) -> String.length k
    | Put (Types.Key k, Types.Value v) ->
  ...
  let arb_cmd state =
    let open QCheck in
    let gen_new_key =
      Gen.map (fun k -> Types.Key k) Gen.small_string
    in
    let key =
...
module MakeLin(KV: KVDirect) : Lin.Spec = struct
  ...
  let api =
    ...
    let value () = Lin.gen_deconstructible value_arb value_to_string value_equal in
    [ val_ "GET" KV.get (t @-> key () @-> returning @@ option @@ value ())
    ; val_ "PUT" KV.put (t @-> key () @-> value () @-> returning @@ option @@ value ())
    ; val_ "DELETE" KV.delete (t @-> key () @-> returning @@ option @@ value ())
...
let tests (module KV:KVDirect) ~count =
  ...
  let module KV_thread = STM_thread.Make(SpecSTM) in

  let module SpecLin = MakeLin(KV) in
  let module KV_lin_thread = Lin_thread.Make(SpecLin) in
  
  KV.name, [ KV_seq.agree_test ~count ~name:(KV.name ^ " STM sequential")
  ; KV_lin_thread.lin_test ~count ~name:(KV.name ^ "Lin thread")
  ; KV_thread.agree_test_conc ~count ~name:(KV.name ^ "STM thread")
  ]