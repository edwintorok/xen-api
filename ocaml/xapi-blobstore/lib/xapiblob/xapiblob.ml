...
let connect () =
  ...
  let* session_id = Session.login_with_password ~rpc ~uname:"root" ~pwd ~version ~originator:name in
...
let disconnect t =
  Session.logout ~rpc:t.rpc ~session_id:t.session_id
...
let list t =
  ...
  List.of_seq (blobs |> List.to_seq |> Seq.map @@ fun (key, _) ->
  Types.Key key)
...
let delete t k =
  ...
  | None -> Lwt.return_none
  | Some (blobref, blob) ->
    let+ () = call t @@ Blob.destroy ~self:blobref in
    Some blob
    
let put t k (Types.Value v) =
  let* r = lookup t k in
  let prev = Option.map snd r in
  let Types.Key name = k in
  let* blobref = call t @@ VM.create_new_blob ~vm:t.vm ~name ~mime_type:"" ~public:false in
