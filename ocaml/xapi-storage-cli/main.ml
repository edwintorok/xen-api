...
let wrap common_opts f =
  wrap_exn common_opts @@ fun () -> let () =  f () in `Ok ()
...
let on_vdi' f common_opts sr vdi =
  on_vdi (fun sr vdi -> let () = f sr vdi in `Ok ()) common_opts sr vdi
...
let vdi_deactivate common_opts sr vdi vm =
  let vm = Vm.of_string vm in
  on_vdi' (fun sr vdi -> Client.VDI.deactivate dbg dbg sr vdi vm) common_opts sr vdi
...
let vdi_deactivate_cmd =
  ...
  let man =
    ...
    @ help
  in
  ( Term.(ret (pure vdi_deactivate $ common_options_t $ sr_arg $ vdi_arg $ vm_arg))
