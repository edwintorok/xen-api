...
let server =
  ...
  in
  let (_: Lwt_unix.signal_handler_id) = Lwt_unix.on_signal Sys.sigterm on_signal in
  let (_: Lwt_unix.signal_handler_id) = Lwt_unix.on_signal Sys.sigint on_signal in
  finished
