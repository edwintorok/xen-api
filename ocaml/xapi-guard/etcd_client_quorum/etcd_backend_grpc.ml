...
let pool sockaddr =
  ...
  in
  let check conn f =
    conn |> H2_lwt_unix.Client.is_closed |> not |> f
  in
  Lwt_pool.create 1
  ~validate
  ~check
  ~dispose:H2_lwt_unix.Client.shutdown @@ fun () -> connect sockaddr
...
let proxy rpc_desc request =
  ...
  let f response_opt_lwt =
    ...
    | None -> Lwt.fail_with "no response?"
    | Some r ->
        r |> Pbrt.Decoder.of_string |> rpc_desc.decode_grpc_response |> Lwt.return_ok
  in
  (* TODO: reuse encoder *)
  let encoder = Pbrt.Encoder.create () in
  Lwt_pool.use persistent_conn @@ fun conn ->
  rpc_desc.encode_grpc_request request encoder;
  let handler = encoder |> Pbrt.Encoder.to_string |> Grpc_lwt.Client.Rpc.unary ~f in
  let* r = Grpc_lwt.Client.call ~service ~rpc:rpc_desc.name ~scheme:"http" ~handler
