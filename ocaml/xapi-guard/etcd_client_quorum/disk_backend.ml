...
let fsync_dir dir =
  (* directories can only be opened RO, it is strange that you can then call a
   ...
   sometimes)
  *)
  with_fd dir [Unix.O_RDONLY] 0 @@ fun fd ->
  Lwt_unix.fsync fd
...
let write_atomic path contents =
  (* we cannot use Lwt_io here, because it doesn't give us access to the
    ...
    *)
  let filename = Fpath.(add_ext ".tmp" path) in (* TODO: unique suffix *)
  Logs.debug (fun m -> m "Attempting to write to %a" Fpath.pp filename);
  let* () = with_fd filename [Unix.O_CREAT; Unix.O_WRONLY; Unix.O_EXCL] 0o700 @@ fun fd ->
...
let read_path key path =
  ...
  (fun () ->
    ...
    Kv_types.default_key_value ~key ~value ()
    |> Option.some
  )
  (function
