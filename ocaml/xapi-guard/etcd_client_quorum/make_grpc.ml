...
module Make (B : KVBackend) = struct
    ...
    |> List.fold_left
         (fun acc (Etcd_service.RPC (rpc, f)) ->
           let unary buffer =
             ...
             let+ result =
               buffer |> Pbrt.Decoder.of_string |> rpc.decode_grpc |> f
             in
             ...
             | Ok reply ->
                 ...
                 Grpc.Status.(v OK)
             | Error status ->
                 Option.iter (fun s -> Pbrt.Encoder.string s encoder) status.message;
                 Grpc.Status.(v ?message:status.message @@ grpc_status_of_code status.code)
             in
             (status, Some (Pbrt.Encoder.to_string encoder))
           in
  ...
  let config =
    H2.Config.
      {default with enable_server_push= false; max_concurrent_streams= 16l}
  ...
  let pp_conn_error ppf = function
    | `Bad_request ->
        Fmt.string ppf "Bad request"
    | `Internal_server_error ->
        Fmt.string ppf "Internal server error"
    | `Exn exn ->
        Fmt.pf ppf "Exception: %a" Fmt.exn exn

  let error_handler _ ?request error start_response =
    let open H2 in
    Logs.warn (fun m ->
        m "error while handling request %a: %a"
          Fmt.(option Request.pp_hum)
          request pp_conn_error error
    ) ;
  ...
  let listen sockaddr =
    let h2_server =
      H2_lwt_unix.Server.create_connection_handler ~config ~request_handler
        ~error_handler
    in
