...
let test_vm_set_nvram_running () =
  with_test_vm (fun __context vm_ref ->
      ...
      Api_server_common.Forwarder.VM.set_NVRAM ~__context ~self:vm_ref ~value:old_nvram ;
      ...
      Api_server_common.Forwarder.VM.set_NVRAM_EFI_variables ~__context ~self:vm_ref
        ~value:new_vars ;
      let read_nvram = Db.VM.get_NVRAM ~__context ~self:vm_ref in
      Alcotest.(check (list (pair string string)))
