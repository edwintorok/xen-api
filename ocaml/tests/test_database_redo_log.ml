...
let is_persistent_event update db =
  match update with
  | RefreshRow _ | PreDelete _ -> false
...
let recover redo_log schema db =
  ...
  let orig_db = db in
  let callback update db = Database.notify update orig_db
  in
  let on_read_db db_ref =
    Db_ref.update_database db_ref (Database.register_callback "forward" callback)
  in
  Redo_log_replay.replay_from_redo_log ~on_read_db redo_log "recovered.db"
    Test_schemas.schema recovered;
...
let check_database_replay msg ~redo_log_name db f =
  ...
  let db =
    Database.register_callback "redo log" Redo_log.database_callback db
  in
  ...
  Alcotest.run_with_args "redo log" prog
    [ ( "dummy"
      , [ ( "dummy"
          , `Quick
          , fun prog ->
            Xapi_stdext_monadic.Opt.iter (fun path -> Db_globs.redo_log_block_device_io := path) prog;
              ...
              check_database_replay "dummy" ~redo_log_name:"test-db-replay" db
                (fun db ->
                  Db_ref.update_database t (fun _ -> db) ;
                  Db_cache_impl.create_row t "VBD"
                    (make_vbd "vmref" "rrr" "uuid")
                    "vm" ;
                  Db_cache_impl.create_row t "VBD"
                    (make_vbd "vmref2" "rrr" "uuid2")
                    "vm2" ;
