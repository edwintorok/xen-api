...
let allocate n =
  Threadpool.create ~name:"authw" Pam.authenticate_start Pam.authenticate_stop n

let free = ignore
(* Threadpool.shutdown*)

let test name execute =
  Test.make_indexed_with_resource ~name ~args:[8] ~allocate ~free
    Bechamel.Test.uniq (fun i ->
      Staged.stage @@ fun pool -> concurrently 32 i (execute pool)
  )
...
let auth_user pool _ = Threadpool.run_in_pool pool auth_pam

let benchmarks =
  Test.make_grouped ~name:"Auth" [test "auth" auth_user; test2 "auth2" auth_pam]
