...
module Rusage = struct
  ...
  (** update measurements lock-free and without allocating,
      ...
      This is similar to a [https://en.wikipedia.org/wiki/Seqlock], but
      doesn't rely on even-odd values, and allows multiple writers to proceed without blocking.
      Readers might have to retry.
   *)
  ...
  (** initialize a measurement record *)
  let make name =
    {
      name
