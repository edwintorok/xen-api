  ...
  Cil.iterGlobals !Cilfacade.current_file (function
    | GFun (g, _) ->
      ...
      | "__VERIFIER_camlparam0" ->
        param0 := Some (g.slocals |> List.filter (fun v -> v.vname = "caml__frame"))
...
let caml_state = lazy (
  match VarQuery.varqueries_from_names !Cilfacade.current_file
  ["Caml_state"] with
  | [VarQuery.Global v], _ -> v
  | _ -> failwith "Missing Caml_state"
)
...
let assert_begins_with_CAMLparam0 f =
  ...
  let similar_varinfo v1 v2 =
    String.equal v1.vname v2.vname
    && CilType.Typ.equal v1.vtype v2.vtype
  in
  let preamble = List.take (List.length caml_frame) f.slocals in
  if not (List.equal similar_varinfo preamble caml_frame) then begin
    ...
    Messages.warn ~category:error_CAMLparam
      "Missing CAMLparam call in function containing 'value' typed parameters/locals: the garbage collector may move these, and they must be registered: preamble: %a" Pretty.(d_list "varinfo" d_varinfo) preamble
  end
