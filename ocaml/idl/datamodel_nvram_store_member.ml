...
let nvram_store_member_operation =
  ...
  [ "membership_change", "changing membership of NVRAM storage cluster member"
  ])
...
let create =
  call ~name:"create" ~doc:"Create a new VM NVRAM storage cluster for the pool"
    ~result:(Ref nvram_store_member, "the newly created NVRAM storage cluster object")
    ~lifecycle ~allowed_roles:_R_POOL_OP
    ~params:
      [ Ref nvram_store, "nvram_store", "the nvram storage cluster to be part of"
      ; Ref _host, "host", "the host to run this NVRAM storage cluster member on"
...
let destroy =
  call ~name:"destroy"
    ~doc:"Destroys an NVRAM storage member"
    ~params:[(Ref nvram_store_member, "self", "the NVRAM storage member to destroy")]
    ~lifecycle ~allowed_roles:_R_POOL_OP
    ()

let set_joined =
  call ~flags:[`Session] ~name:"set_joined"
    ~doc:"Add or remove member from NVRAM storage cluster"
    ~lifecycle ~params:
...
let t =
  create_obj ~name:nvram_store_member ~descr:"VM NVRAM storage server"
    ~doccomments:[] ~gen_events:true ~in_db:true ~lifecycle ~persist:PersistEverything
    ~in_oss_since:None ~messages_default_allowed_roles:_R_POOL_OP
    ~gen_constructor_destructor:false
    ~contents:
      ([
        uid nvram_store_member ~lifecycle
      ; field ~qualifier:StaticRO ~lifecycle ~ty:(Ref nvram_store) "nvram_store"
      ...
      ; field ~qualifier:StaticRO ~lifecycle ~ty:(Map (String, String))
