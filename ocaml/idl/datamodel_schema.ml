...
let of_datamodel () =
  ...
  let column obj f =
    let issetref =
      match f.Datamodel_types.ty with
      | Datamodel_types.Set (Datamodel_types.Ref _) ->
          true
      | _ ->
          false
    in
    let is_many_to_many f =
      ...
      && Datamodel_utils.Relations.classify api
           (this, Datamodel_utils.Relations.other_end_of api this)
         = (`Many, `Many)
    in
    let ty =
      match f.Datamodel_types.ty with
      | Datamodel_types.Set _ ->
          Type.Set
      | Datamodel_types.Map (_, _) ->
          Type.Pairs
      | _ ->
          Type.String
    in
    ...
    ; (* NB we always regenerate one-to-many Set(Ref _) fields *)
      persistent=
        f.Datamodel_types.field_persist
        && (is_many_to_many f
           || (not issetref)
           || f.Datamodel_types.field_ignore_foreign_key
           )
  ...
  ; one_to_many=
      List.fold_left
        (add_relation is_one_to_many)
        ForeignMap.empty
        (Dm_api.relations_of_api Datamodel.all_api)
  ; many_to_many=
      List.fold_left
        (add_relation is_many_to_many)
        ForeignMap.empty
        (Dm_api.relations_of_api Datamodel.all_api)
  }
