    ...
    ~versioned_params:
      ([
       ...
       ; {
         ...
         ; param_doc=
             "true if xapi is automatically joining new pool members to the \
              cluster"
         ; param_release= kolkata_release
         ; param_default= None
         }
...
let destroy =
  call ~name:"destroy"
    ...
    ~params:[(Ref _cluster, "self", "the Cluster to destroy")]
    ~lifecycle ~allowed_roles:_R_POOL_OP
    ~errs:Api_errors.[cluster_does_not_have_one_node; cluster_stack_in_use]
    ()

let get_network =
  call ~name:"get_network"
    ~doc:
      ("Returns the network used by the cluster for inter-host communication, "
  ...
  call ~name:"pool_create"
    ...
    ~versioned_params:
      ([
         ...
         ; param_name= "network"
         ; param_doc=
             "the single network on which corosync carries out its inter-host \
              communications"
         ; param_release= kolkata_release
         ; param_default= None
         }
...
let pool_force_destroy =
  call ~name:"pool_force_destroy"
    ~doc:
      "Attempt to force destroy the Cluster_host objects, and then destroy the \
       Cluster."
    ~params:[(Ref _cluster, "self", "The cluster to force destroy.")]
    ~lifecycle ~allowed_roles:_R_POOL_OP
    ~errs:Api_errors.[cluster_force_destroy_failed]
    ()

let pool_destroy =
  call ~name:"pool_destroy"
    ~doc:
      "Attempt to destroy the Cluster_host objects for all hosts in the pool \
  ...
  create_obj ~name:_cluster ~descr:"Cluster-wide Cluster metadata"
    ...
    ~contents:
      ([
       ...
       ; field ~qualifier:DynamicRO ~lifecycle ~ty:(Set (Ref _cluster_host))
           "cluster_hosts"
       ...
       ; field ~qualifier:StaticRO ~lifecycle ~ty:String "cluster_token"
           ~default_value:(Some (VString ""))
           "The secret key used by xapi-clusterd when it talks to itself on \
            other hosts"
      ...
      @ [
          field ~qualifier:StaticRO ~lifecycle ~ty:Bool "pool_auto_join"
            ~default_value:(Some (VBool true))
            "True if automatically joining new pool members to the cluster. \
             This will be `true` in the first release"
        ; field ~qualifier:StaticRO ~lifecycle:lifecycle_timeout ~ty:Float
            "token_timeout"
            ~default_value:(Some (VFloat Constants.default_token_timeout_s))
            "The corosync token timeout in seconds"
        ; field ~qualifier:StaticRO ~lifecycle:lifecycle_timeout ~ty:Float
            "token_timeout_coefficient"
            ~default_value:
              (Some (VFloat Constants.default_token_timeout_coefficient_s))
            "The corosync token timeout coefficient in seconds"
        ; field ~qualifier:StaticRO ~lifecycle
            ~ty:(Map (String, String))
            "cluster_config" ~default_value:(Some (VMap []))
            "Contains read-only settings for the cluster, such as timeouts and \
             other options. It can only be set at cluster create time"
