...
let create =
  call ~name:"create" ~doc:"Create a new VM NVRAM storage cluster for the pool"
    ...
    ~params:
      [ Ref _network, "network", "the network to use for communication"
      ]
    ()

let destroy =
  call ~name:"destroy"
    ~doc:"Destroys an NVRAM storage cluster"
    ~params:[(Ref nvram_store, "self", "the NVRAM storage cluster to destroy")]
    ~lifecycle ~allowed_roles:_R_POOL_OP
    ()

let t =
  create_obj ~name:nvram_store ~descr:"VM NVRAM storage cluster"
    ...
    ~contents:
      ...
      ; field ~qualifier:StaticRO ~lifecycle ~ty:(Ref _network) "network" "Reference to the network object"
      ...
      ; field ~qualifier:DynamicRO ~lifecycle ~ty:(Set (Ref nvram_store_member)) "nvram_store_members"
        "A list of the NVRAM_store_member objects associated with this NVRAM_store"
