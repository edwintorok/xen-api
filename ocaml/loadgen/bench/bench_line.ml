...
let count_line acc _ _ _ = acc + 1
...
module OCamllex = struct
  ...
  let reset (_, _, off) = off := 0

  let rec line_ocamllex buff f ((buff', lexbuf, _) as t) acc =
    assert (buff' == buff) ;
    match Find_line.line lexbuf with
    | 0 ->
        acc
    | pos ->
        let start = Lexing.lexeme_start lexbuf in
        let acc = f acc buff start (pos - start) in
        line_ocamllex buff f t acc

  let line_ocamllex buff f t = line_ocamllex buff f t 0
end
...
let test_line name ~allocate ~reset f =
  ...
  for _ = 1 to repeat do
    reset t ;
    let n = f test_data count_line t in
    assert (n = 10)
  done

let benchmarks =
  Test.make_grouped ~name:"countlines"
    [
