...
let[@inline never] log_exn name exn =
  let bt = Printexc.get_raw_backtrace () in
  let thunk, finally = Opentelemetry.Trace.with_' name ignore in
  thunk () ;
...
let[@inline always] bounded_add_ignore_exn ~__FUNCTION__ queue t f =
  if is_recording t then
    try Bounded.add queue @@ f () with exn -> log_exn __FUNCTION__ exn
...
let[@inline always] add_log t f =
  if is_recording t then
    try
      let log = f () in
      if log.Opentelemetry_proto.Logs.severity_number >= Severity_number_info
      then
        Logs.emit [log]
      else
        Bounded.add t.logs log
    with exn -> log_exn __FUNCTION__ exn
