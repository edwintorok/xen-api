...
let on_progress_cli ~total =
  ...
  fun completed progress ->
    ...
    if completed = total then Printf.eprintf "\n%s%!" P.(summarise p)

let run_with_progress t _scope ?(on_progress = on_progress_cli) ~callback ~total
    tasks =
   ...
   *)
  Trace.with_ ~force_new_trace_id:true "run_with_progress" @@ fun scope ->
...
let batched_run_or_cancel t ?(batch_size = 32) ?on_progress apifns =
  (* default batch_size= 32 = 2*Dom0 vCPUs *)
  let total = List.length apifns in
  Trace.with_ "batched_run_or_cancel" ~attrs:[("total", `Int total)]
  @@ fun scope ->
  ...
  let callback completed task =
    let outcome = Task.result t (Hashtbl.find taskref_to_task task) in
  ...
  run_with_progress t scope ?on_progress ~callback ~total tasks ;
...
let rec wait_no_active_tasks ?on_progress t ~host =
  ...
  | _ ->
      ...
      let () =
        ...
        run_with_progress t scope ?on_progress ~total:(List.length tasks)
          ~callback:(fun _ _ -> [])
          tasks
      in
