...
let create_backend' ~filename () : (module BACKEND) =
  (module struct
      ...
      let rec loop () =
        ...
        | Some msg ->
            let () =
              ...
              msg |> List.iter (fun item -> encoder item t) ;
    ...
    let make encoder filename =
      let file = Out_channel.open_bin (filename ^ ".otel") in
      let t = {file; ch= Event.new_channel (); send_ns= Atomic.make 0} in
      let thread = Thread.create (handle encoder) t in
      (thread, t)

    let trace =
      make Opentelemetry_proto.Trace.encode_pb_resource_spans
        (filename ^ ".trace")

    let metrics =
      make Opentelemetry_proto.Metrics.encode_pb_resource_metrics
        (filename ^ ".metrics")

    let logs =
      make Opentelemetry_proto.Logs.encode_pb_resource_logs (filename ^ ".logs")
