...
module Backend : Collector.BACKEND = struct
  ...
  let scope_logs t = List.iter log_record t.Opentelemetry_proto.Logs.log_records
  ...
  let span t =
    let duration =
      (Int64.sub t.end_time_unix_nano t.start_time_unix_nano |> Int64.to_float)
      *. 1e-9
    in
  ...
  let tick () = Atomic.get on_tick |> AList.get |> List.iter execute
...
let with_setup ?enable () f =
  Collector.with_setup_debug_backend ?enable (module Backend) () f
