...
let rec stable_localhost_free_pages scope t ~host =
  let rec loop scope delay =
    ...
    if v1 <> v0 then begin
      ...
      v1
  in
  Trace.with_ ~scope __FUNCTION__ @@ fun scope -> loop scope 0.005
...
let ensure_vm_clones t ~vm n purpose =
  ...
  let new_vms =
    Trace.with_ ~scope "Cloning new VMs"
      ~attrs:[("n", `Int (List.length tasks))]
    @@ fun _ -> Api.batched_run_or_cancel t tasks |> check_tasks
  in
  ...
  in
  let (_ : _ list) =
    Trace.with_ ~scope "Hard shutdown existing VMs"
      ~attrs:[("n", `Int (List.length tasks))]
    @@ fun _ -> Api.batched_run_or_cancel t tasks |> check_tasks
  in
