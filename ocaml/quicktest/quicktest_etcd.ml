...
let live =
  Config.Live.
  { advertise_client_urls = [Uri.make ~host:"localhost" ~port ~scheme:"http" ()]
  (* empty value not valid here *)
  ; initial_advertise_peer_urls = [Uri.make ~host:"localhost" ~port:peer_port ~scheme:"http" ()]
  }

let local =
  Config.Local.
  { config = { name = "localtest"; enable_v2 = false; log_level = Some Config_field.DEBUG; log_output = Some Stdout }
...
let test_start_stop_direct config () =
  ...
  let stdout, stdin, stderr = Unix.open_process_full (Filename.quote_command cmd args) env in
