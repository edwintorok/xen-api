...
let make_cmd str = Cmd.of_string str |> R.error_msg_to_invalid_arg
...
let opam_provided_packages_of_lockfile ~repositories ~lockfile =
  ...
  OS.Cmd.run_out ~env
    Cmd.(opam_switch_cmd "show" % "-f" % "depexts" % p lockfile)
  |> out_string_lines
...
module Stage = struct
  ...
  let v ?alias ?tag ?platform from_image lst =
    {
      alias
    ; dockerfile= Dockerfile.(from ?alias ?tag ?platform from_image @@@ lst)
    }
  ...
  *)
  let with_ro_mount ~from ~source ~target t =
    let mount =
      Dockerfile.mount_bind ?from:from.alias
        ~source:Fpath.(home // source |> to_string)
        ~target ()
    in
...
let with_build_cache ~id ~target t =
  Dockerfile.with_mounts
    [
      Dockerfile.mount_cache ~uid:Config.container_uid ~gid:Config.container_gid
        ~id
...
let dune_workspace = Fpath.v "/home/opam/workspace"
...
let dune_build ?(release = false) ~argname ~source_build_id targets =
  ...
  @@ with_user_cache
       ...
       ; cmd_run Cmd.(v "cd" % p target) (* FIXME: xapi specific *)
       ...
       ; cmd_run
           Cmd.(v "sudo" % "make" % "-C" % "scripts" % "install" % "DESTDIR=/")
       ; cmd_run
           Cmd.(
             v "cp"
             % p Fpath.(v "run.sh")
             % p Fpath.(home / "prefix" / "run.sh")
           )
       ]
...
let main =
  ...
  let installer = distro |> installer_of_distro in
  ...
  let main_stage =
    ...
    Stage.v image
      Dockerfile.
        ...
        ; installer ["etcd"; "/usr/sbin/ip"; "stunnel"; "strace"; "openssl"; "python3-six"]
        ; Dockerfile.arg argname
        ; Stage.copy_from ~from:duniverse_stage ~source:duniverse_path
            ~target:duniverse_path
        ; Dockerfile.run
            "mkdir -p /home/opam/workspace/source_ro \
             /home/opam/workspace/_build"
