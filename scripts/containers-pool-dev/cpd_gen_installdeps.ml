...
let install_opam_packages ~repository ~lockfile ~uniqueid =
  Command.run
    [
      Command.v
        Cmd.(v "opam" % "repository" % "set-url" % "default" % repository)
...
let build_xapi ~uniqueid =
  ...
  Command.run
  @@ Dune.build ~release:true ~uniqueid
       ~source:Fpath.(v ".")
       [
         "xapi.install"
       ; "xe.install"
       ; "message-switch-unix.install"
       ; "forkexec.install"
       ]
...
let opam_deps_id =
  OS.Arg.(opt ["opam-deps-id"] ~absent:None @@ some string |> Option.get)

let gitdescribe =
  OS.Arg.(opt ["gitdescribe"] ~absent:None @@ some string |> Option.get)

let repository =
  OS.Arg.(opt ["repository"] ~absent:None @@ some string |> Option.get)
            ...
            [
              Command.v
                Cmd.(
                  ...
                  % "-p"
                  % p Dune.workspace
                  % p Fpath.(home / ".cache" / "dune" / "db" / "files")
                )
            ]
        ; install_opam_packages ~repository ~lockfile ~uniqueid:opam_deps_id
        ; Dockerfile.copy ~from:duniverse_stage ~src:[duniverse_path]
            ~dst:duniverse_path ()
