...
# dune can install libraries and several other files into the right locations
	dune install --destdir=$(DESTDIR) --prefix=$(PREFIX) --libdir=$(LIBDIR) --mandir=$(MANDIR) \
		xapi-client xapi-schema xapi_consts xapi-cli-protocol xapi-datamodel xapi-types \
		...
		xapi-rrd xapi_inventory clock \
		xapi-stdext-date xapi-stdext-encodings xapi-stdext-pervasives xapi_stdext_std xapi-stdext-threads xapi_stdext_unix xapi-stdext-zerocheck
# docs
